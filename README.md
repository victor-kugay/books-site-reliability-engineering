...
- [Addressing Cascading Failures](#addressing-cascading-failures)
- [Reliable Product Launches at Scale](#reliable-product-launches-at-scale)
...
> Автоматизация развивается по следующему пути:
>
> - Нет автоматизации. Например, Для главной базы данных восстановление после отказа выполняется вручную на всех площадках.
> - Управляемая извне автоматизация, характерна для данного типа систем. Например, SR-инженер создал сценарий для восстановления в своей "домашней" директории.
> - Управляемая извне общая автоматизация. Например, SR-инженер добавляет поддержку базы данных в общий сценарий восстановления, которым пользуются все.
> - Управляем изнутри автоматизация, специфичная для данного типа систем. Например, база данных поставляется с собственным сценарием восстановления.
> - Системы, для которых автоматизация не требуется. Например, база данных обнаруживает проблемы и автоматически преодолевает отказы без вмешательства человека.
...
> Ошибка - плата за знания. Дэвин Кэрреуэй

> Подготовленный постмортем должен пройти одобрение и быть опубликован. На практике это означает, что команды создают заготовку постмортема собственными силами, а затем просят группу старших инженеров оценить его на предмет завершенности. Критерии оценки могут включать следующее:
>
...
> Не оставляйте ни одного постмортема без оценки. Нерассмотренный постмортем все равно как не существующий. Чтобы убедиться, что каждая заготовка прошла рассмотрение, мы поощряем регулярные семинары для разбора постмортем. На этих встречах важно доводить до завершения все текущие дискуссии, фиксировать возникшие идеи и придавать окончательный вид документам.

> Внедрение культуры постмортема:
>
> - Постмортем месяца
> - Группа Google+
> - Книжные клубы по постмортемам
> - Колесо неудачи

> Одна из главных трудностей при внедрении культуры постмортема в организации состоит в том, что некоторые могут оспорить их значимость, ссылаясь на затраты на подготовку. Следующие меры помогут преодолеть это затруднени:
>
...
> Мы сделали критичность основным понятием системы RPC и хорошо поработали, интегрировав ее в механизмы управления, чтобы ее можно было учитывать, когда происходит реакция на перегрузки:
>
...
> - Планирование производительности. Хорошее планирование производительности может снизить вероятность каскадного сбоя. Кго нужно объединить с тестированием производительности для того, чтобы определить нагрузку, при которой сервер начнет давать сбой

## Reliable Product Launches at Scale

> Вопросы к запуску проекта:
>
> - Архитектура:
...
>   - Получили ли вы все необходимые вычислительные ресурсы для поддержки сетевого трафика?
> - Типы сбоев:
>   - Имеются ли в проекте единые точки сбоев?
>   - Как вы справляетесь с недоступностью зависимостей?
> - Поведение клиента:
>   - Ваш клиент имеет экспоненциальную задержку при сбое?
>   - Ваш клиент распределяет во времени автоматические запросы?
> - Ручные процессы:
